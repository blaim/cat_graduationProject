...
for l in test_url:
    ...
    try:
        ...
        while True:
            ...
            if driver.find_element(By.CSS_SELECTOR,'#content > div > div > div.styled__Wrap-sc-1j5nm8l-0.dWqXbC > div.styled__PaginWrap-sc-1u1e15y-0.eOczmr > ul > li:nth-last-child(1) > button').get_attribute('disabled'):
                break
            # 다음 페이지가 있다면 버튼을 눌러서 이동한다.
            else:
    ...
    print(dict)
    # json 파일 경로는 추후 수정
    with open('../static/js/data.json', 'w' ,encoding='utf-8-sig')as f:
        for i in fd:
            dict.insert(j, i)
            print(dict[j])
            j += 1
