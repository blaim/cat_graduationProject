...
def get_room_information(url):
    ...
    try:
        ...
        big_image_url = driver.execute_script(script);
        big_image_url = big_image_url[5:-2]
        small_images_urls = driver.execute_script(scripts);
        for i in range(4):
            small_images_urls[i] = small_images_urls[i][5:-2]
        ...
        informations.append(big_image_url)
        informations.append(small_images_urls)
        ...
        doc = {
            'URL' : url,
            '큰 사진 url': big_image_url,
            '작은 사진들 url': small_images_urls,
            '공인중개사': agent,
            '월세': monthly_pay,
            '관리비':care_cost,
            '단기임대':Short_rental,
